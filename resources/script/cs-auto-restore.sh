...
# 1. get my_wiki
WIKI_DB_FILE=$(cd ./mountpoint/backup_restore/mariadb/ &&  ls -Arth *.sql.gz | tail -n 1)
echo Get latest Mediawiki file: $WIKI_DB_FILE

# 2. get image file
IMG_FILE=$(cd ./mountpoint/backup_restore/mediawiki/ && ls -Arth *images*  | tail -n 1)
...
# # 4. Get Matomo
# MTM_FILE=$(cd ./cs/mountpoint/backup_restore/mariadb/ && ls *images* -Arth | tail -n 1)
# echo Get latest Matomo file: $MTM_FILE
# echo ./cs-restore.sh -m ./mountpoint -d $MTM_FILE -i $IMG_FILE -t my_wiki

# # 5. Get Gitea
# GIT_FILE=$(cd ./cs/mountpoint/backup_restore/mariadb/ && ls *images* -Arth | tail -n 1)
